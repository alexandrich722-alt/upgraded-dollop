...
export function Badge({ children, color = 'cyan' }: { children: ReactNode; color?: string }) {
  const colors: Record<string, string> = {
    ...
    amber: 'bg-accent-amber/12 text-accent-amber border border-accent-amber/15',
    muted: 'bg-nova-surface text-nova-dim border border-nova-border',
    default: 'bg-nova-surface text-nova-dim border border-nova-border',
  };
  return <span className={`badge ${colors[color] || colors.cyan}`}>{children}</span>;
}

export function ProgressBar({ value, color = 'cyan', size = 'md', className = '' }: { value: number; color?: string; size?: 'sm' | 'md' | 'lg'; className?: string }) {
  const colors: Record<string, string> = {
    cyan: 'bg-accent-cyan', violet: 'bg-accent-violet', emerald: 'bg-accent-emerald',
    gold: 'bg-accent-gold', rose: 'bg-accent-rose', blue: 'bg-accent-blue', amber: 'bg-accent-amber',
  };
  const glow: Record<string, string> = {
    ...
    amber: 'shadow-[0_0_8px_rgba(251,191,36,0.3)]',
  };
  const heights = { sm: 'h-1', md: 'h-2', lg: 'h-2.5' };
  return (
    <div className={`w-full bg-nova-surface rounded-full overflow-hidden ${heights[size]} ${className}`} role="progressbar" aria-valuenow={Math.round(value)} aria-valuemin={0} aria-valuemax={100}>
...
export function PriorityBadge({ priority }: { priority: string }) {
  const map: Record<string, { color: string; label: string }> = {
    critical: { color: 'rose', label: 'Critical' }, high: { color: 'amber', label: 'High' },
...
export function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { color: string; label: string }> = {
    ...
    paused: { color: 'amber', label: 'Paused' }, archived: { color: 'muted', label: 'Archived' },
    todo: { color: 'muted', label: 'To Do' }, in_progress: { color: 'amber', label: 'In Progress' },
